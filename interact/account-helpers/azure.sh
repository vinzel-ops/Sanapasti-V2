...
#
echo -e -n "${Green}Please enter your resource group name: (Default 'sanapasti'), press enter) \n>> ${Color_Off}"
read resource_group

if [[ "$resource_group" == "" ]]; then
    echo -e "${Blue}Selected default option 'sanapasti'${Color_Off}"
    resource_group="sanapasti"
fi
...
echo -e -n "${Green}Would you like to configure connection to an sanapasti Pro Instance? Y/n (Must be deployed.) (optional, default 'n', press enter) \n>> ${Color_Off}"
read ans

if [[ "$ans" == "Y" ]]; then
    echo -e -n "${Green}Enter the sanapasti pro instance name \n>> ${Color_Off}"
    read appliance_name
...
echo $data | jq > "$SANAPASTI_PATH/accounts/$title.json"
echo -e "${BGreen}Saved profile '$title' successfully!${Color_Off}"
$SANAPASTI_PATH/interact/sanapasti-account $title
